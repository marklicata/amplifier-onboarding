...
class StreamingBundleRunner:
    ...
    def __init__(self, bundle_path: str):
        ...
        if AMPLIFIER_AVAILABLE:
            ...
            print(f"Amplifier not available", file=sys.stderr)
    ...
    async def initialize_session(self):
        ...
        try:
            ...
            print(f"Bundle loaded successfully", file=sys.stderr)
        except Exception as e:
            print(f"ERROR loading bundle: {type(e).__name__}: {str(e)}", file=sys.stderr)
            import traceback
        ...
        try:
            print(f"Starting prepare() - this will download git modules...", file=sys.stderr)
            prepared = await foundation.prepare()
            print(f"Prepare completed successfully", file=sys.stderr)
        except Exception as e:
            print(f"ERROR during prepare: {type(e).__name__}: {str(e)}", file=sys.stderr)
            import traceback
            traceback.print_exc(file=sys.stderr)
            stream_event('step', {
                'step': 'preparing',
