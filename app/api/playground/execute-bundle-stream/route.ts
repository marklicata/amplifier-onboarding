...
interface ExecuteBundleStreamRequest {
  bundleId: string;
  bundlePath: string;
  prompt: string;
}
...
/**
 * POST /api/playground/execute-bundle-stream
 * Streaming execution endpoint for bundles using Server-Sent Events
 */
export async function POST(request: NextRequest) {
  try {
    const body: ExecuteBundleStreamRequest = await request.json();
    const { bundleId, bundlePath, prompt } = body;

    if (!bundleId || !bundlePath || !prompt) {
      return new Response('bundleId, bundlePath, and prompt are required', { status: 400 });
    }
    ...
    console.error('Execute bundle stream API error:', error);
    return new Response(
...
/**
 * GET /api/playground/execute-bundle-stream
 * Health check
 */
export async function GET() {
  return new Response(
    JSON.stringify({
      status: 'ok',
      service: 'amplifier-playground-execute-bundle-stream',
